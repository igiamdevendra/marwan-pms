...
const Users = () => {
  ...
  const handleFormSubmit = (values, { resetForm }) => {
    if (editingUser) {
      dispatch(editUserAsync({ ...editingUser, ...values }));
    } else {
      const userWithId = {
        ...values,
        id: uuidv4(),
  ...
  return (
    <div className="p-3 bg-gray-100 min-h-screen w-full">
      <div className="max-w-4xl mx-auto bg-white shadow-md rounded-lg p-6">
        ...
        />
        <div>
          <Input value={searchText} onChange={(e) => setSearchTest(e.target.value)} type="search" placeholder="Search" className="border border-gray-300 rounded-md p-2" />
        </div>
        </div>
        <div className="mt-5">
          {isLoading ? (
            ...
            <CustomTable columns={columns} data={filteredUsers} className="w-full" />
          )}
        </div>
      </div>
      <CustomModal
